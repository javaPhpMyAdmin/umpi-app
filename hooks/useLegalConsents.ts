 ...
 *
 * Replica of the web app's hook (umpi-web/src/hooks/useLegalConsents.ts),
 * adapted to the mobile AuthContext signature (session + isLoading) and the
 * mobile query conventions. Uses the SAME tables and RPC already deployed in
 * the shared Supabase database — nothing new is created here.
 *
 * WHY VERSIONED: the current legal copy ships with the app under
 * LEGAL_VERSION (see lib/legalContent.ts). When the copy changes, bump
 * LEGAL_VERSION — every user who accepted only an older version is re-gated
 * by LegalConsentGate until they accept the new text. Old rows in
 * `legal_consents` stay untouched as the audit trail.
 */
import { useMutation, useQuery, useQueryClient } from '@tanstack/react-query';
import { useAuth } from '@/contexts/AuthContext';
import { supabase } from '@/lib/supabase';
import { LEGAL_DOCUMENTS, LEGAL_VERSION } from '@/lib/legalContent';
...
/**
 * True when the user has accepted the CURRENT version of every legal
 * document. Older-version acceptances don't count — a version bump re-gates
 * the user until they accept the new text.
 */
export function hasAcceptedCurrentVersion(
  consents: LegalConsent[] | undefined,
): boolean {
...
export function useLegalConsentGate() {
  ...
  return {
    needsConsent: !!userId && !hasAcceptedCurrentVersion(consents),
